...
| `platforms/android/gradle.properties` | +1 propriedade `armsx2.applicationId`, hoje em `com.armsx2` | a troca para `io.armada.sx2` está pronta e **adiada até a campanha de medição fechar**: mudar o id zera a pasta de dados (BIOS, saves, drivers, configuração), e repetir isso no meio da campanha trocaria dado por atrito. O `namespace` (pacote Java) segue `com.armsx2` em definitivo — trocá-lo renomearia centenas de arquivos e quebraria todo merge futuro, sem ganho visível para o usuário |
...
| `platforms/android/.../runtime/MainActivityRuntime.kt` | copia a pasta `assets/shaders` para o data root | instala o preset ATR do fork no mesmo diretório que o seletor de Shader Chain já varre; nenhum caminho paralelo de configuração |
