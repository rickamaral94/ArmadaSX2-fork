Warning: truncated output (original token count: 93385)
Total output lines: 9118
...
void GSDeviceVK::SubmitCommandBuffer(VKSwapChain* present_swap_chain)
{
	...
	if (present_swap_chain)
	{
		...
		hygiene.atr_enabled = GSConfig.ShaderChainEnabled &&
			GSConfig.ShaderChainPreset.find("amaral-temporal-reconstruction") != std::string::npos;
...
#define CHECKED_CREATE(create_fn, create_struct, output_struct) \
	...
	buf_create.size = 4;
	buf_create.usage…43385 tokens truncated…ex, bool sharpen_only, const std::array<u32, NUM_CAS_CONSTANTS>& constants)
{
