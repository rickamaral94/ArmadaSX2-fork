 ...
 */
private val BASE_EN: Map<String, String> = mapOf(
    "about.title" to "About app",
    "friends.title" to "Friends",
    "friends.explain" to "Link Discord to show what you're playing and see which friends are in ArmadaSX2. Uses your existing Discord friends — ArmadaSX2 keeps no account and runs no server. Only while the app is open.",
    ...
    "friends.playingNow" to "In ArmadaSX2 now",
    "friends.nobody" to "None of your Discord friends are in ArmadaSX2 right now.",
    "friends.notify" to "Notify me in game",
    "friends.notify.desc" to "Show a message over the game when a friend starts playing ArmadaSX2.",
    ...
    "about.armsx2.title" to "ARMSX2 project",
    "about.armsx2.description" to "ArmadaSX2 is a fork of ARMSX2, the Android port this app is built from.",
    "about.pcsx2.title" to "PCSX2 project",
    "about.pcsx2.description" to "ARMSX2 in turn is built on the open-source PCSX2 emulator.",
    ...
    "update.signatureWarning" to "Alpha builds are signed with a throwaway key that changes every build, so this will not install over the current one. Uninstall first — and back up Android/data/com.armsx2/files/ before you do, or you lose memory cards and settings.",
    ...
    "update.checkOnLaunch.desc" to "Automatically check for a new ArmadaSX2 version each time the app starts.",
    "update.includeNightly" to "Offer alpha versions",
    "update.includeNightly.desc" to "Offer alpha (pre-release) builds. On by default, because every ArmadaSX2 release so far is an alpha — with this off the check will always say you are up to date. Turn it off once stable releases exist and you only want those.",
    ...
    "app.bootLogo.desc" to "Play the bundled intro video when the app starts.",
    ...
    "fixes.crop.desc" to "Trims pixels from the edges of the image before scaling, to hide garbage or black bars a TV would have cut off. Measured in native PS2 pixels.",
    ...
    "games.exit.title" to "Exit ArmadaSX2?",
    ...
    "games.coverRegion.library" to "Library",
    ...
    "games.quickLoad.confirmTitle" to "Set up quick loading",
    "games.quickLoad.confirmBody" to "This copies the disc's files into ARMSX2's internal storage so the game can load them directly.\n\nSpace needed: about %1\$s\nFree right now: %2\$s\n\nYou can remove them later by holding the entry it creates and choosing Remove quick-load files.",
    "games.quickLoad.confirmLowSpace" to "There may not be enough room — this needs about %1\$s and only %2\$s is free.",
    "games.quickLoad.continue" to "Choose the ELF",
    "games.quickLoad.remove" to "Remove quick-load files",
    "games.quickLoad.removeConfirm" to "Delete the extracted disc files for this entry? The game itself is untouched, and you can set it up again any time.",
    "games.quickLoad.removed" to "Quick-load files deleted.",
    "games.quickLoad.removeFailed" to "Couldn't delete those files.",
    "games.quickLoad.working" to "Extracting disc files…",
    "games.quickLoad.extractFailed" to "Couldn't read that disc. Quick loading needs a plain .iso — a .chd or .cso won't work.",
    ...
    "games.quickLoad.done" to "Ready — %d files extracted. Launch \"%s\" from your library to play.",
    "games.elfDisc" to "Set disc for this ELF",
    "games.elfDisc.set" to "Disc paired with this ELF. Launch the ELF to play.",
    "games.elfDisc.failed" to "Couldn't read that ELF — pick the .elf file, not the disc.",
    "games.categories" to "Categories",
    "games.categories.title" to "Categories",
    "games.categories.subtitle" to "A game can be in as many as you like.",
    "games.categories.none" to "No categories yet. Make one below.",
    "games.categories.new" to "New category",
    "games.categories.newHint" to "Name",
    "games.categories.add" to "Create",
    "games.categories.rename" to "Rename",
    "games.categories.delete" to "Delete category",
    "games.categories.deleteConfirm" to "Delete \"%s\"? The games stay in your library.",
    "library.category.all" to "All games",
    "library.category.filter" to "Category",
    ...
    "hotkeys.exitToLauncher.description" to "When a game was launched from another app (e.g. ES-DE), Close Game returns to that app instead of the ArmadaSX2 library.",
    ...
    "memcard.backups" to "Backups",
    "memcard.backups.auto" to "Automatic card backups",
    "memcard.backups.auto.help" to "Saves a copy of each memory card every time you start a game, so a card that goes bad can be put back. Only saves a copy when the card is readable and has actually changed.",
    "memcard.backups.empty" to "No backups of this card yet. One is saved each time you start a game.",
    "memcard.backups.good" to "Verified",
    "memcard.backups.now" to "Back up now",
    "memcard.backups.restore" to "Restore",
    "memcard.backups.restore.body" to "\"%s\" will be replaced with the copy saved on %s.\n\nA copy of the card as it is right now is saved first, so this can be undone.",
    "memcard.backups.restore.confirm" to "Restore this backup?",
    "memcard.backups.suspect" to "Unreadable when saved",
    "memcard.backups.title" to "Backups of %s",
    "memcard.backups.vmRunning" to "Stop the game before restoring a memory card. The console keeps its own picture of the card while it runs, and would write over the restored copy.",
    "memcard.recovery.body" to "\"%s\" cannot be read.\n\nThere is a backup saved on %s. Restore it before starting?",
    "memcard.recovery.ignore" to "Start anyway",
    "memcard.recovery.restore" to "Restore backup",
    "memcard.recovery.title" to "Memory card unreadable",
    ...
    "memcard.delete.body" to "Remove \"%s\" from ArmadaSX2?\n\nThis deletes the card that ArmadaSX2 keeps in its own memory-card folder. If you imported it from a file on your device, that original file is not touched.",
    ...
    "memcard.perGame.menu" to "Memory Cards",
    ...
    "overlay.quickMenuSide.label" to "Quick Menu Side",
    "overlay.quickMenuSide.description" to "Which edge the in-game menu slides in from. Also on the in-game quick menu itself.",
    "overlay.quickMenuSide.left" to "Left",
    "overlay.quickMenuSide.right" to "Right",
    "overlay.osdPosition.label" to "Stats Position",
    "overlay.osdPosition.description" to "Which corner the performance stats sit in. Also on the in-game quick menu.",
    "overlay.osdPosition.topLeft" to "Top Left",
    "overlay.osdPosition.topRight" to "Top Right",
    "overlay.osdPosition.bottomLeft" to "Bottom Left",
    "overlay.osdPosition.bottomRight" to "Bottom Right",
    ...
    "overlay.osdSize.description" to "Size of the on-screen display drawn over the game — FPS, stats and notifications. Does not affect the app's menus. 100% = PCSX2's default; ArmadaSX2 ships 65% so it doesn't crowd a handheld screen.",
    ...
    "patches.disclaimer" to "Cheats and patches written for older cores (AetherSX2 / NetherSX2 1.7) can freeze, crash or glitch games on this newer core. If a game misbehaves, turn patches and cheats off and test again before reporting it — if it only happens with them on, it's the patch.",
    "patches.bundled.header" to "Bundled with ArmadaSX2",
    "patches.bundled.explain" to "These ship inside ArmadaSX2 and are applied to this game automatically — you didn't install them. Ones with no name are always on and can't be switched off individually.",
    ...
    "patches.hostFs.folder" to "host: reads from the \"hostfs\" folder in your ARMSX2 data folder. Put the files a game expects there — for the Biohazard Outbreak quick-load ELF, that is where its data goes.",
    ...
    "perf.clamp.followVu0" to "Same as VU0",
    ...
    "perf.eeFpuClamping.description" to "FPU overflow/rounding accuracy. Exact reproduces the EE's multiplier and divider bit-for-bit, for the few games that check results exactly — it is the slowest setting by some way. Restart the game to apply.",
    ...
    "perf.fix.compatPatches.desc" to "Applies the per-game compatibility patches bundled with ArmadaSX2 — the fixes some games need to boot or render correctly. Same idea as GameDB Fixes above, and safe to leave on. Does NOT enable widescreen or cheats; those live in the Patches screen. Takes effect on the next boot.",
    ...
    "perf.fix.gamedbFixes.desc" to "Master switch for the compatibility fixes below, plus the automatic per-game fixes from ArmadaSX2's game database. Leave on unless troubleshooting.",
    ...
    "perf.vu1Clamping.description" to "Clamping for VU1 alone. Same as VU0 follows the VU Clamping setting; anything else overrides it, so a game that needs VU0 accurate can leave VU1 cheaper. Restart the game to apply.",
    "perf.vu1Clamping.label" to "VU1 Clamping",
    "perf.vu1RoundMode.label" to "VU1 Round Mode",
    "perf.vuClamping.description" to "VU float clamping — VU0, and VU1 unless VU1 Clamping overrides it. Exact adds the VU's own arithmetic and status flags on top of Extra+Sign — the most accurate and slowest setting. Restart the game to apply.",
    ...
    "renderer.upscaler.label" to "Upscaler",
    "renderer.upscaler.off" to "Off",
    "renderer.upscaler.description" to "How the low-resolution frame is stretched to the screen. FSR 1 is AMD's, and adds its own sharpening pass. SGSR 1 is Qualcomm's, built for Adreno, and does the whole thing in one pass. Off is a plain bilinear stretch. Only one can run, and either one replaces CAS.",
    ...
    "renderer.forceMaliFbFetch.description" to "Testing option — affects MediaTek Mali and Mali-G57 on the Vulkan renderer only, and is ignored on every other GPU and renderer. ArmadaSX2 normally disables framebuffer fetch on these drivers because they hand back stale or empty destination colour, which shows up as black or missing textures. Mali has no hardware dual-source blend, so with fetch off the emulator has to re-read the destination through a per-primitive barrier — very slow in blend-heavy games. Turn this on to test whether your driver is actually affected: if it is fine you should get a large speedup; if it is not you will see black or missing textures, so turn it back off. OFF by default. Restart the game to apply.",
    "renderer.forceMaliFbFetch.label" to "Force Framebuffer Fetch (MediaTek Mali)",
    ...
    "renderer.gsDumpSeq.label" to "Capture GS Sequence (benchmark)",
    "renderer.gsDumpSeq.description" to "Records a fixed number of frames of GPU commands (.gs) to the snaps folder. Replaying it is deterministic, so renderer settings can be A/B tested on identical frames. Stops on its own; the file is much larger than a single-frame dump.",
    "renderer.gsDumpSeq.queued" to "Recording %d frames — close this menu and play the scene you want measured. It stops by itself and saves to the snaps folder.",
    "renderer.gsDumpSeq.length.label" to "Sequence length (frames)",
    "renderer.gsDumpSeq.length.description" to "How many frames the sequence capture records. 120 is two seconds at 60 Hz and is enough for an A/B, because both runs replay the same stream. 600 produces files too large to move off the device.",
    ...
    "renderer.importTexturePack.bootFirst" to "Boot a game first so ArmadaSX2 knows the serial.",
    ...
    "secondScreen.tile.title" to "Game",
    "secondScreen.tile.fps" to "FPS",
    "secondScreen.tile.speed" to "Speed",
    "secondScreen.tile.battery" to "Battery",
    "secondScreen.tile.clock" to "Clock",
    "secondScreen.tile.achievements" to "Achievements",
    "secondScreen.tile.aspect" to "Aspect",
    "secondScreen.tile.slot" to "Save slot",
    "secondScreen.tile.hide" to "Hide panel",
    "secondScreen.tile.macro1" to "Macro 1",
    "secondScreen.tile.macro2" to "Macro 2",
    "secondScreen.tile.macro3" to "Macro 3",
    "secondScreen.tile.macro4" to "Macro 4",
    "secondScreen.layout" to "Panel layout",
    "secondScreen.layout.desc" to "Pick which boxes the panel shows and in what order. Arrows move a box earlier or later; the grid fills left to right.",
    "secondScreen.layout.columns" to "Columns",
    "secondScreen.layout.reset" to "Reset layout",
    ...
    "network.mode.label" to "Network mode",
    "network.mode.online" to "Online (Sockets)",
    "network.mode.host" to "Host local game",
    "network.mode.join" to "Join local game",
    "network.mode.description" to "Online plays through the internet against game servers. Host and Join link this device with others directly, as if the consoles were wired to one switch — for the roughly 57 PS2 games that have a built-in LAN or System Link mode.",
    "network.localLink.help" to "⚠ Only works with the roughly 57 PS2 games that shipped with their own LAN or System Link mode — for example Gran Turismo 4, TimeSplitters 2, Midnight Club 3, Star Wars Battlefront and the SOCOM games. In the game's multiplayer menu look for LAN, System Link or Network Play. If it instead asks you to sign in to a server, that game was online-only and this cannot revive it.\n\nEasiest on one Wi-Fi network or a phone hotspot. It also works over the internet, but only if the host can be reached from outside — a VPN such as Tailscale on both devices is the simplest way, since mobile data and most home routers block incoming connections. The host starts the game, then guests enter the host's address with the same port and room code. Each player needs their own copy of the game.",
    "network.localLink.limits" to "Not a netplay feature: it does not sync save states or let two players share one copy of a game. It emulates a network cable between consoles, so only games with their own LAN mode can use it.",
    "network.localLink.gamesList" to "Which games support this?",
    "network.localLink.gamesList.desc" to "Opens Wikipedia's list of PS2 LAN games",
    "network.localLink.wifiSettings" to "Wi-Fi and hotspot settings",
    "network.localLink.wifiSettings.desc" to "Connect the devices before starting the game",
    "network.localLink.hostAddress" to "This device's address",
    "network.localLink.hostAddress.desc" to "Give one of these to the other players to enter on their device.",
    "network.localLink.noAddress" to "Not connected",
    "network.localLink.address" to "Host IPv4 address",
    "network.localLink.address.desc" to "The address shown on the host device, for example 192.168.1.42, or a VPN address such as 100.101.102.103. A hostname also works if you have one.",
    "network.localLink.roomCode" to "Room code",
    "network.localLink.roomCode.desc" to "Use the same 4-12 letter or digit code on every device. It keeps two nearby sessions apart; it is not security, so use it on a network you trust.",
    "network.localLink.port" to "Local Link port",
    "network.localLink.port.desc" to "Use the same port on every device (1024-65535). There is no automatic negotiation.",
    "network.localLink.peerId" to "Local peer ID",
    "network.localLink.peerId.desc" to "Set automatically and different on every device, because it decides the console's network address. The host is always 1.",
    "network.localLink.notSet" to "Not set",
    "network.localLink.generate" to "Generate",
    "network.localLink.roomCode.invalid" to "⚠ The room code must be 4-12 letters or digits. Until it is, the console's network adapter stays switched off and games will say it is not connected.",
    ...
    "perf.fg.mode.label" to "Frame Generation",
    "perf.fg.mode.auto" to "Auto",
    "perf.fg.mode.description" to "When generated frames may be presented. Auto engages only while pacing is steady; 2x asks for one generated frame per rendered frame. Both stand down when the emulation is too slow to smooth honestly.",
    "perf.fg.warning" to "Frame generation improves perceived smoothness. It does NOT make the emulation run faster.",
    ...
    "setup.systemDir.intro" to "ArmadaSX2 stores memory cards, save states, configs, and shader data in app-private storage by default. ",
    ...
    "setup.welcome.heading" to "Welcome to ArmadaSX2 setup!",
    ...
    "savestate.slot.empty" to "Empty",
    "renderer.upscaler.label" to "Display upscaler",
    ...
    "secondScreen.tile.gpuLoad" to "GPU load",
    "secondScreen.tile.frameTime" to "Frame time",
    ...
    "osd.temps" to "Device temperatures",
    "secondScreen.layout.tileHeight" to "Tile height (0 = fit text)",
    "secondScreen.tile.notHere" to "Not this screen",
    "secondScreen.displays.reset" to "Re-enable skipped displays",
    "secondScreen.topBar" to "Status bar",
    "secondScreen.topBar.desc" to "Show the clock and battery in a bar across the top instead of as tiles",
    "secondScreen.background.custom" to "Custom",
    "secondScreen.background.choose" to "Choose panel image",
...
/**
 * The canonical English table.
 *
 * Split so the frame-generation strings can be ABSENT from the play build rather than merely
 * unreachable in it. They are the only user-visible text in the app naming a third-party product,
 * and a Kotlin map literal is compiled in whatever any runtime flag says — a BuildConfig.LSFG
 * guard on the UI keeps the rows from being drawn while every one of these strings still sits in
 * the play dex in plain text, findable by anyone who searches for them.
 *
 * See I18nLsfg.kt in the github and play source sets.
 */
val EN: Map<String, String> = BASE_EN + LSFG_EN
